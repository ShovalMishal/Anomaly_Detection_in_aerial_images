...
def parse_args():
    ...
    train = args.train
    return data_source, ood_class_names, train


def ood_detector_stage(data_source, train, ood_class_names):
    # Create dataloaders
    ...
    # Train classifier
    model = train_classifier(in_dist_train_dataloader, in_dist_val_dataloader, num_labels=len(in_dist_train_dataloader.dataset.classes), train=train)

    # Create output folders
    os.makedirs("./upper_bound_output/OOD_Detector", exist_ok=True)
    os.makedirs("./upper_bound_output/Classifier", exist_ok=True)
    # Create OOD detector
    OOD_detector = ODINOODDetector(output_dir="./upper_bound_output/OOD_Detector")
    ...
    plot_confusion_matrix(confusion_matrix=res_confusion_matrix, classes=in_dist_train_dataloader.dataset.classes,
                          normalize=True,
                          output_dir="./upper_bound_output/Classifier")
    plot_confusion_matrix(confusion_matrix=res_confusion_matrix, classes=in_dist_train_dataloader.dataset.classes,
                          normalize=False,
                          output_dir="./upper_bound_output/Classifier")
    ...
    with open("./upper_bound_output/OOD_Detector/val_dataset_scores_and_labels.json", 'w') as f:
        json.dump(labels_scores_dict, f, indent=4)
    # Plot OOD detector results
    plot_graphs(scores=scores, labels=labels, path="./upper_bound_output/OOD_Detector", title="OOD stage",
...
if __name__ == '__main__':
    data_source, ood_class_names, train = parse_args()
    ...
    create_ood_id_dataset(src_root=os.path.join(data_source, "val"),
                          ...
                          ood_classes=ood_class_names)
    ood_detector_stage(data_source, train, ood_class_names)
